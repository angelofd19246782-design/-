...
1. **Real licensed packaging photos** (Wikimedia Commons + Open Food Facts,
   both CC-licensed) — for products where an exact-or-close brand shot
   exists, e.g. Borjomi, Alenka, Krakovskaya, Doktorskaya, Сгущёнка
   Верховского, Doshirak, Maille pickles, Adjika, Twarog, etc.

2. **Clean SVG placeholders** rendered client-side with the product name
   and a "DEMO IMAGE" watermark — for products where no good licensed
   studio photo exists (Kefir, Ryazhenka, Kvass, Halva, Zefir, Krasny
   Oktyabr, Salami, Olivier, Selyodka, Holodets). Marked with
   `image_url: 'placeholder'` in `db/seed.js`. The placeholder is
   generated by `placeholderSVG()` in `public/js/catalog.js`.

> **Before going to production:** replace placeholders (and ideally all
> images) with real photos from the client's own suppliers / brand
> media-kits. The bundled fetcher (`npm run fetch-images`) makes this a
> 1-step swap once URLs are in hand — see below.
